...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    public String recommendWhoToFollow(String name) {
        ...
        for (int i = 0; i < userCount; i++)
        {
            if (!mainUser.equals(users[i]) && !mainUser.follows(users[i].getName()))
            {
                tempMutual = users[i].countMutual(mainUser);
                if (tempMutual > maxMutual)
                {
                    maxMutual = tempMutual;
                    indexOfMax = i;
                }
            }
        }
        ...
        return users[indexOfMax].getName();
    }

    /** Computes and returns the name of the most popular user in this network: 
     *  The user who appears the most in the follow lists of all the users. */
    public String mostPopularUser() {
        int[] mostPopular = new int[userCount];
        ...
        for (int i = 0; i < userCount; i++)
        {
            if (mostPopular[indexOfMax] < mostPopular[i]) indexOfMax = i;
        }
        return users[indexOfMax].getName();
    }
